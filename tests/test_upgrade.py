...
if __name__ == '__main__':
    ...
    convert("flyem-flylight.json", lambda x: upgrade_matches(legacy_model.CDSMatches(**x)))
    convert("flyem-flylight-vnc.json", lambda x: upgrade_matches(legacy_model.CDSMatches(**x)))
    convert("flylight-flyem.json", lambda x: upgrade_matches(legacy_model.CDSMatches(**x)))
    convert("flylight-flyem-vnc.json", lambda x: upgrade_matches(legacy_model.CDSMatches(**x)))
    convert("pppresult.json", lambda x: upgrade_matches(legacy_model.PPPMatches(**x)))
    convert("pppresult-vnc.json", lambda x: upgrade_matches(legacy_model.PPPMatches(**x)))
